...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        activities = orm.TeachingActivity.objects.all()

        activities_by_referenceID = {}
        for activity in activities:
            activity_list = activities_by_referenceID.setdefault(activity.reference_id, [])
            activity_list.append(activity)

        for referenceID, activity_list in activities_by_referenceID.items():
            if len(activity_list) > 1:
                print "WARNING: Found activity list with multiple activities. Reference %s" % referenceID

                activity_list.sort(key=lambda a: a.id)

                # The one with the lowest ID was probably the first created.
                primary_activity = activity_list[0]

                # We will keep a list of the activity years so that we don't have anything unexpected occur to the
                # activities themselves when we make the transfer.
                activity_years_to_transfer = []
                for activity in activity_list[1:]:
                    for activity_year in activity.years.all():
                        activity_years_to_transfer.append(activity_year)

                for activity_year in activity_years_to_transfer:
                    activity_year.teaching_activity = primary_activity
                    activity_year.save()
