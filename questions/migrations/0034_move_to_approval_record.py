...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for question in orm.Question.objects.all():
            if question.approver:
                approval = orm.ApprovalRecord()
                approval.question = question
                approval.approver = question.approver
                approval.status = question.status
                approval.save()
                all_approvals.append(approval)

        for approval in all_approvals:
            approval.date_completed = datetime.datetime.now()
            approval.save()
    ...
    def backwards(self, orm):
        ...
        for question in orm.Question.objects.all():
            try:
                approval = question.approval_records.latest('date_completed')
            except orm.ApprovalRecord.DoesNotExist:
                pass
            else:
                question.approver = approval.approver
                question.status = approval.status
                question.save()
