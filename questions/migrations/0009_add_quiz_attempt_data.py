...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        all_attempts = {}
        for q_attempt in orm.QuestionAttempt.objects.all():
            student_attempt = all_attempts.setdefault(q_attempt.student, {})
            
            added = False
            for date_range in student_attempt.keys():
                if date_range[0] <= q_attempt.date_attempted <= date_range[1]:
                    student_attempt[date_range].append(q_attempt)
                    added = True
                    break
            ...
            date_range = [q_attempt.date_attempted, q_attempt.date_attempted + datetime.timedelta(seconds=10)]
            student_attempt[date_range] = [q_attempt, ]

        for student in all_attempts:
            for date_range in all_attempts[student]:
                quiz_attempt = orm.QuizAttempt()
                quiz_attempt.student = student
                quiz_attempt.date_submitted = date_range[0]

                quiz_attempt.save()

                for question_attempt in all_attempts[student][date_range]:
    ...
    def backwards(self, orm):
        ...
        for quiz_attempt in orm.QuizAttempt.objects.all():
            for question_attempt in quiz_attempt.questions.all():
                question_attempt.student = quiz_attempt.student
                question_attempt.date_attempted = quiz_attempt.date_submitted
                question_attempt.save()
    ...
    models = {
        ...
        u'questions.questionattempt': {
            ...
            'quiz_attempt': ('django.db.models.fields.related.ForeignKey', [], {'blank': 'True', 'related_name': "'questions'", 'null': 'True', 'to': u"orm['questions.QuizAttempt']"}),
