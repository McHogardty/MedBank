...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for question in orm.Question.objects.all():
            # Get the latest assigned record for this question
            try:
                latest_approval_record = question.approval_records.latest("date_assigned")
            except orm.ApprovalRecord.DoesNotExist:
                continue

            question.status = latest_approval_record.status
            question.date_assigned = latest_approval_record.date_assigned
            question.date_completed = latest_approval_record.date_completed
            question.approver = latest_approval_record.approver
            question.save()

            latest_approval_record.delete()
    ...
    def backwards(self, orm):
        ...
        for question in orm.Question.objects.all():
            if question.date_assigned:
                new_approval_record = orm.ApprovalRecord()
                new_approval_record.status = question.status
                new_approval_record.date_assigned = question.date_assigned
                new_approval_record.date_completed = question.date_completed
                new_approval_record.approver = question.approver
                new_approval_record.question = question

                new_approval_record.save()
    ...
    models = {
        ...
        u'questions.approvalrecord': {
            ...
            'date_assigned': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime.now'}),
