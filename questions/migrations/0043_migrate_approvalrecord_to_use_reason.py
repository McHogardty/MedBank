...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        approval_records = orm.ApprovalRecord.objects.all()
        FLAGGED_TYPE = 1
        approval_record_content_type = orm['contenttypes.ContentType'].objects.get(app_label="questions", model="approvalrecord")

        for record in approval_records:
            if record.reason:
                new_reason = orm.Reason()
                new_reason.body = record.reason
                new_reason.creator = record.approver
                new_reason.date_created = record.date_completed
                new_reason.reason_type = FLAGGED_TYPE
                new_reason.related_object_content_type = approval_record_content_type
                new_reason.related_object_id = record.pk
                new_reason.save()
